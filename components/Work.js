

export default function Work() {
  return (
    <div>
        <section className="spacer container text-center">
           
                <h2 className='md:mt-12 md:w-3/4 mx-auto text-gray-900 text-5xl md:text-8xl'>My Recent Work</h2>
            
           
            <p className='text-black py-6 md:w-3/4 mx-auto'>Below are examples of my work, including both client projects and personal creative work.</p>
            
           
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 items-center justify-center mx-auto'>
                 <iframe src="https://www.youtube.com/embed/ExLO0SIyBls?autoplay=0&mute=0&controls=1&showinfo=1&rel=0"
                    className="w-full h-96 mx-auto"
                    frameBorder="0"
                    allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    loading="lazy">
                </iframe>
                 <iframe src="https://www.youtube.com/embed/Sf6DDi1XpO4?autoplay=0&mute=0&controls=1&showinfo=1&rel=0"
                    className="w-full h-96 mx-auto"
                    frameBorder="0"
                    allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    loading="lazy">
                </iframe>
                <iframe src="https://www.youtube.com/embed/C7cVBtoExGg?autoplay=0&mute=0&controls=1&showinfo=1&rel=0"
                    className="w-full h-96 mx-auto"
                    frameBorder="0"
                    allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    loading="lazy">
                </iframe>
                <iframe src="https://www.youtube.com/embed/deWgIHzAqQ4?autoplay=0&mute=0&controls=1&showinfo=1&rel=0"
                    className="w-full h-96 mx-auto"
                    frameBorder="0"
                    allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    loading="lazy">
                </iframe>
            </div>
        </section>
    </div>
  )
}

